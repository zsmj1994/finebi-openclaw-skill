...
1. 先读取 `references/dashboard-id-resolution-flow.md`，解析唯一的 `dashboardId`。不要把用户输入名称直接当成 id。
2. 如需读取组件数据，再读取 `references/dashboard-widget-data-flow.md`，按配置里的组件 id 取数。
3. 再读取看板或组件的真实数据。优先使用 `get-widget-data` 获取关键组件数据，必要时使用 `export-dashboard-pdf` 获取完整快照。
4. 最后再做摘要、卡片或定时播报。
