...
class Config:
    ...
    def decision_url(self, path: str) -> str:
        ...
        Handles common base_url patterns:
        ...
        - webroot URL:  ``http://host/webroot/decision``        → webroot root = ``http://host/webroot``
        """
        base = self.base_url
        base_lower = base.lower()
        for context in ["/WebReport", "/webroot"]:
            if context.lower() in base_lower:
                idx = base_lower.index(context.lower())
                webroot = base[: idx + len(context)]
                return f"{webroot}/{path.lstrip('/')}"
        # Fallback: derive from BI-style URL by stripping the last path segment
        parts = base.rsplit("/", 1)
